...
def score_candidate(
    ...
    components = {
        "quality": {
            ...
            "rvol_strength": round(rvol_strength, 1) if rvol_strength is not None else None,
            "price_response": round(response_strength, 1) if response_strength is not None else None,
            "leadership": round(leadership_score, 1),
            "rs_strength": round(rs_strength, 1) if rs_strength is not None else None,
            "momentum_3m": round(momentum_strength, 1) if momentum_strength is not None else None,
            "high_proximity_52w": round(high_proximity_strength, 1) if high_proximity_strength is not None else None,
